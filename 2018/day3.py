...
def get_claims():
    day3_input = advent_of_code.get_day_input(3)
    r = re.compile('#(?P<ID>\d+) @ (?P<left_edge>\d+),(?P<top_edge>\d+): (?P<width>\d+)x(?P<height>\d+)')
    return [m.groupdict() for m in r.finditer(day3_input)]
...
if __name__ == '__main__':
    day3_claims = get_claims()
    claimed_fabric = get_claimed_fabric(day3_claims)

    print(get_overlapping_area_of_claims(claimed_fabric))
    print(get_perfect_claim_id(day3_claims, claimed_fabric))
